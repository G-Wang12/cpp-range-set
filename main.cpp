...
int main(int argc, char **argv)
{
    ...
    // --no-points to indicate we we don't want to add the points from points.txt into range_set
    // --no-ranges to indicate we we don't want to add the rangess from range.txt into range_set
    ...
    if (use_ranges)
    {
        ...
        if (f)
        {
            ...
            // uint64_t x;
            // while (f >> x) {
            //   if (x <= numeric_limits<uint32_t>::max()) rs.add_point((uint32_t) x);
            // }
        }
    }
